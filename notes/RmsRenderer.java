  ...
  public static GroupItem getRmsGroupHeader ( byte[] data, BuddyList buddyList ) {
    ...
    try {
      for ( int c = 0; c < childsCount; c++ ) {
        ...
        groupChild = t_Id.indexOf ( '@' ) != -1 ? new BuddyItem ( t_Id ) : new ServiceItem ( t_Id, t_Id );
        offset += t_Int;
        t_Int = DataUtil.get16 ( data, offset );
        offset += 2;
  ...
  public static byte[] getRmsData ( GroupItem groupHeader ) {
    ...
    try {
      for ( int c = 0; c < groupHeader.getChildsCount (); c++ ) {
        // offset = 0;
        groupChild = ( BuddyItem ) groupHeader.childs.elementAt ( c );
        titleData = StringUtil.stringToByteArray ( groupChild.getJid (), true );
        itemData = new byte[ 9 + titleData.length + StringUtil.stringToByteArray ( groupChild.getNickName (), true ).length + StringUtil.stringToByteArray ( groupChild.getSubscription (), true ).length ];
        /** Title **/
